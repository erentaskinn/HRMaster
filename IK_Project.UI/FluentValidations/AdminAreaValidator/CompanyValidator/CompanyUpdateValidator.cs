...
{
    public class CompanyUpdateValidator :AbstractValidator<AdminCompanyUpdateVM>
    {
        public CompanyUpdateValidator()
        {
            ...
            RuleFor(x => x.DealStartDate)
                .NotEmpty().WithMessage("Deal start date is required.")
                .GreaterThanOrEqualTo(DateTime.Today).WithMessage("Deal start date must be today or a future date.");
